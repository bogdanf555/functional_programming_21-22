...
view (Interval interval) =
    let
        ...
        endDateText = if interval.end == Nothing then "Present" else dateToString <| Maybe.withDefault (Date.onlyYear 2020) interval.end
        ...
        finalChildElements = 
            case intervalLength of
                Just (years, months) -> 
                    List.append childElements [
                        p [ class "interval-length"] [
                            text <| ("Years: " ++ String.fromInt years ++ ", Months: " ++ String.fromInt months)
                        ]
                    ]
                Nothing -> childElements
    in
        div attributes finalChildElements
